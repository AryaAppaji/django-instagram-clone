...
def loginUser(request):
    ...
    if user is not None:
        ...
        APIKey.objects.filter(
            name=user.username
        ).delete()  # Ensure user has no old keys
        ...
        api_key, key = APIKey.objects.create_key(
            name=user.username, expiry_date=expiration_time
        )
        ...
        return Response(
            {
                "user": UserSerializer(
                    user
                ).data,  # Return the username instead of the user object
                "api_key": key,
                "status": True,
            },
...
def logoutUser(request):
    ...
    if not api_key_header:
        return Response(
            {"error": "API key required"}, status=status.HTTP_400_BAD_REQUEST
        )

    try:
        # Extract API key from the Authorization header
        api_key = api_key_header.split(" ")[
            1
        ]  # Expects format like "Api-Key YOUR_API_KEY"
        ...
        return Response(
            {"error": "Invalid or missing API key"}, status=status.HTTP_400_BAD_REQUEST
        )
